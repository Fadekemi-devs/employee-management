...
employee_updated = Counter(
    "employee_updated_total",
    "Total employees updated"
)

employee_deleted = Counter(
...
def edit_employee(employee_id):
    if request.method == "POST":
        ...
        position = request.form["position"]
        salary = request.form["salary"]
        update_employee(employee_id, name, email, department, position, salary)
        employee_updated.inc()  # moved AFTER update_employee so it only counts successes
        return redirect("/")
...
@app.route("/metrics")
def metrics_endpoint():  # renamed to avoid conflict with the 'metrics' variable above
    return Response(
        generate_latest(),
        mimetype="text/plain"
    )
